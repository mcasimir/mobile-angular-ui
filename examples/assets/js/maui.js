...
angular.module("maui.directives.contentFor", []).directive("contentFor", [
  "$sce", function($sce) {
    return {
      compile: function(e, attrs, transclude) {
        return {
          pre: function(scope, iElement, iAttrs, controller) {
            var html, targetScope, varname;
            html = void 0;
            targetScope = void 0;
            varname = void 0;
            ...
            while (targetScope.$parent) {
              if ((targetScope.hasOwnProperty ? targetScope.hasOwnProperty(varname) : targetScope[varname])) {
                break;
              } else {
  ...
  function() {
    return {
      ...
      link: function(scope, elem, attrs) {
        ...
        if ((_base = scope.$root).sidebars == null) {
          _base.sidebars = {};
        }
        if ((_base1 = scope.$root.sidebars)[_name = attrs.sidebar] == null) {
          _base1[_name] = {
            visible: false
          };
        }
        elem.addClass("sidebar");
        return scope.$on("sidebar.toggle", function(e, name, status) {
          if (name === attrs.sidebar) {
            if (status) {
              return elem.addClass("in");
            } else {
      ...
      controller: [
        "$rootScope", "$scope", function($rootScope, $scope) {
          if ($rootScope.sidebars == null) {
            $rootScope.sidebars = {};
          }
          $rootScope.showSidebar = function(name) {
            var _base;
            if ((_base = $rootScope.sidebars)[name] == null) {
              _base[name] = {};
            }
            $rootScope.sidebars[name].visible = true;
            return $rootScope.$broadcast("sidebar.toggle", name, $rootScope.sidebars[name].visible);
          };
          $rootScope.hideSidebar = function(name) {
            var _base;
            if ((_base = $rootScope.sidebars)[name] == null) {
              _base[name] = {};
            }
          ...
          $rootScope.toggleSidebar = function(name) {
            var _base;
            ...
            $rootScope.sidebars[name].visible = !$rootScope.sidebars[name].visible;
            return $rootScope.$broadcast("sidebar.toggle", name, $rootScope.sidebars[name].visible);
          };
          $rootScope.hideSidebars = function(options) {
            var name, sidebar, _ref, _ref1, _results;
            if (options == null) {
              options = {};
            }
            _ref = $rootScope.sidebars;
            _results = [];
            for (name in _ref) {
              sidebar = _ref[name];
              if ((!options.except) || (_ref1 = !name, __indexOf.call(options.except, _ref1) >= 0)) {
                sidebar.visible = false;
                _results.push($rootScope.$broadcast("sidebar.toggle", name, sidebar.visible));
              } else {
...
angular.module("maui.directives.tabs", []).directive('toggleTab', [
  function() {
    return {
      restrict: "A",
      scope: {},
      link: function(scope, elem, attrs) {
        var gid_id, myGid, myId;
        gid_id = (attrs.toggleTab || attrs.target || attrs.href).split(".");
        myGid = gid_id[0];
        myId = gid_id[1];
        if (myGid[0] === "#") {
          myGid = myGid.slice(1);
        }
        scope.$on("maui.tabs.activated", function(e, gid, id) {
          var parent;
          parent = elem.parent();
          if (gid === myGid) {
            if (id === myId) {
        ...
        return elem.on("click tap", function(e) {
  ...
  function() {
    return {
      ...
      link: function(scope, elem, attrs) {
        var gid_id, myGid, myId;
        gid_id = (attrs.tabPane || attrs.id).split(".");
        myGid = gid_id[0];
        myId = gid_id[1];
        elem.addClass("tab-pane");
        return scope.$on("maui.tabs.activate", function(e, gid, id) {
          if (gid === myGid) {
            if (id === myId) {
              elem.addClass("active");
              return scope.$root.$broadcast("maui.tabs.activated", myGid, myId);
            } else {
...
angular.module("maui", ['ngTouch', 'ngAnimate', 'ui.bootstrap.accordion', 'ui.bootstrap.collapse', 'ui.bootstrap.dropdownToggle', 'maui.directives.activeLink', 'maui.directives.contentFor', 'maui.directives.sidebar', 'maui.directives.tabs']).run([
  "$rootScope", function($rootScope) {
    return $rootScope.$on("$locationChangeSuccess", function(e, newPath, oldPath) {
